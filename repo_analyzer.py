...
def get_files(root_path, max_depth=2, exclude=None):
    ...
    def scan(path, depth):
        for p in sorted(path.iterdir()):
            if p.name.startswith(".") or p.suffix in exclude: continue
            rel = "./" + str(p.relative_to(base))
            if p.is_file(): files[rel] = p
            if p.is_dir() and depth < max_depth: scan(p, depth + 1)
...
def build_map(path, base, index, depth=0, max_depth=2):
    ...
    items = sorted(p for p in pathlib.Path(path).iterdir() if not p.name.startswith("."))
...
sys_prompt = open(prompt_path, encoding='utf-8').read() if os.path.exists(prompt_path) else ""
...
for rel, path_obj in files.items():
    ...
    try:
        ...
        agent = Agent(history, model_name="cometapi/gemini-3-flash-preview", preset="custom", prompt=sys_prompt)
        resp = agent.chat()
